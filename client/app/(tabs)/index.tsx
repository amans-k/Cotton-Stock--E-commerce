...
export default function Home() {
  ...
  return (
    <SafeAreaView className='flex-1' edges={['top']}>
     <Header title='CottonStock' showMenu showCart showLogo />
          ...
          <View className='mb-6'>
            ...
            <ScrollView horizontal showsHorizontalScrollIndicator={false} className='w-full'>
              {categories.map((cat:any) => (
                <Categorieitem key={cat.id} item={cat} isSelected={false} onPress={()=> router.push({pathname: "/shop", params: {categories: cat.id === 'all' ? '' : cat.name}})}/>
              ))}
            </ScrollView>
            </View>
          {/* popular Products */}
